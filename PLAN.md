...
- **dplyr ≥ 1.2 idioms:** `.by=` over `group_by()` for one-off grouping; `across()`; `join_by()`; `reframe()` for multi-row summaries; `replace_values()`/`replace_when()`; `when_any()`/`when_all()`. For ACS variable→label recoding use **`case_when()`** — the pinned **dplyr is 1.2.1** (confirmed at renv snapshot), in which `recode_values()` is unavailable and `case_match()` is soft-deprecated (matches fhfh's standardization).
